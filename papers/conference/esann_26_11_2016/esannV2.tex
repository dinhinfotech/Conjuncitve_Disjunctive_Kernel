...
Gene-disease associations networks can have nodes with high degrees.
Unfortunately these networks cannot be meaningfully processed by decomposition
kernels based on exact neighborhood matches. Instead we propose to decompose
...
\textit{K-core Decomposition.}  The K-core decomposition \cite{kcore}
 partitions the node set in two groups on the basis of the degree of
each vertex w.r.t. a threshold degree $D$. We use the vertex partition to
induce an edge partition: edges that have endpoints in the same part are
marked as {\em conjunctive} edges, while edges with endpoints in different
parts are marked as {\em disjunctive} edges. We apply the K-core decomposition
iteratively considering only the graph induced by the conjunctive edges until
no node has a degree greater than $D$.

\textit{Clique Decomposition}: 

The clique decomposition (inspired by \cite{clique}) identifies all the
cliques (completely connected subgraphs) with a number of nodes greater than a
threshold size $C$. For each clique a new 'representative' node is added. The
endpoints of all edges incident on the clique's nodes are transferred to the
representative node. Finally all edges with both endpoints in the clique are
removed.
