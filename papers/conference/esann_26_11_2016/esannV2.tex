...
\textit{K-core Decomposition.} 
The K-core decomposition \cite{kcore} procedure partitions the node set in two
groups on the basis of the degree of each vertex w.r.t. a threshold degree
$D$. We use the vertex partition to induce an edge partition: edges that
have endpoints in the same part are marked as {\em conjunctive} edges, while
edges with endpoints in different parts are marked as {\em disjunctive} edges.
We apply the K-core decomposition iteratively using a series of threshold
values $\{D_0, D_1, \ldots, D_l \}$. 

\textit{Clique Decomposition}: 

The clique decomposition (inspired by \cite{clique}) procedure partitions the 


begins with finding the set of cliques
$L$ in $G$ that have size no less than $C$. For each clique in $L$, first, we
add a new node to $G$. We then connect the new node to clique's nodes with
disjunctive edges and to all neighborhood of clique's nodes at distance 1 with
conjunctive edges. Finally, we remove all conjunctive edges that have end
points at one of the clique nodes.
